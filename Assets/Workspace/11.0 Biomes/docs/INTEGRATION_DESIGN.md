...
1. **Q10 ignition + decay sinks.** ✅ **Shipped** (`Biome.compute:229`): the linear `decompRate = rate·(0.5+temp)` is now `rate·pow(2.74,(temp−0.5)·decompTempSpan)` so decomposition is near-zero when cold and *explosive* when hot → travelling fertility fronts instead of a smooth ramp. Sinks shipped as Waste `decay 0.001` + Nutrient `decay 0.0005`; Oxygen breathes via relaxation (→0.8) rather than decay. `decompRate` is clamped. The front now *travels* via the **diurnal sun (row 6, ✅ shipped)**; a sharper Temperature `diffuseRate` (0.96) + reduced agent metabolic heat give it the headroom to move rather than pin a static max-fertility disk.
2. **Diurnal "sun" forcing.** ✅ **Shipped** (`BiomeInjector` procedural source, `AddDiurnalSunSource`): a warm `MaxToward` Gaussian sweeps Temperature L→R, phase from the **neuron playhead** (`NeuronFiringSource.CurrentFrame/FrameCount`) so one firing-blob playthrough = one day and the `index=0` loop reset lands on sunrise (free-running `SimStepCount`/`periodSteps` fallback when no OSC index). The master off-equilibrium pump: moves the hot zone → re-aims flow, re-fires Q10, drives evaporation. Kept **indirect by design** — no sim reads Temperature; the sun reads out purely through flow/humidity/Q10/permeability, so headroom (heat balance) + gradient (temp `diffuseRate`) are the tuning levers, not a direct chemotaxis read. Built as a pre-Step injector stamp, `Biome.Step` untouched.
