...
namespace Biomes
{
    public class ScreenLayoutPreview : EditorWindow
    {
        ...
        [MenuItem("Tools/Screen Layout Preview (11.0)")]
        static void Open()
        {
        ...
        void OnGUI()
        {
            ...
            // Composite texture aspect (default to 1:1 if no texture)
            ...
            for (int i = 0; i < _layout.screens.Count; i++)
            {
                ...
                if (_layout.activeScreenIndex == i)
                {
                    Color highlightCol = Color.white;
                    highlightCol.a = 0.8f;
                    DrawRectOutline(new Rect(screenRect.x - 2, screenRect.y - 2, screenRect.width + 4, screenRect.height + 4), highlightCol, 2f);
                }
        ...
        {
            // Try getting the texture from the screen material
            if (_layout != null && _layout.screenMaterial != null)
            {
                var tex = _layout.screenMaterial.GetTexture("_UnlitColorMap");
                if (tex != null) return tex;
            }
            // Fallback: try compositeOutMat on SimManager
            if (_simManager != null && _simManager.compositeOutMat != null)
            {
                var tex = _simManager.compositeOutMat.GetTexture("_UnlitColorMap");
                if (tex != null) return tex;
            }
            return null;
        }
