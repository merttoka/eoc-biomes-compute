...
namespace Biomes
{
    ...
    public struct ScreenConfig
    {
        ...
        [Tooltip("Enable/disable this screen at runtime")]
        public bool enabled;
    }
    ...
    public class ScreenLayout : MonoBehaviour
    {
        ...
        public string rtAssetFolder = "Workspace/11.0 Biomes/render_textures";
        public List<RenderTexture> rtAssets = new List<RenderTexture>();

        [Header("Runtime")]
        public int activeScreenIndex = -1;
        ...
        {
#if UNITY_EDITOR
            if (screens.Count == 0) { Debug.LogError("[ScreenLayout] Generate screens first."); return; }
        ...
        {
            ClearScreenQuads();
            if (screenMaterial == null) { Debug.LogError("[ScreenLayout] No screenMaterial."); return; }
            ...
            for (int i = 0; i < screens.Count; i++)
            {
                ...
                quad.SetActive(cfg.enabled);

                RenderTexture rt = (i < rtAssets.Count && rtAssets[i] != null) ? rtAssets[i] : CreateRuntimeRT(cfg);
        ...
        // ─── Runtime screen cycling (for MFT) ───

        [Button("Cycle Active Screen")]
        public void CycleActiveScreen()
        {
            activeScreenIndex++;
            if (activeScreenIndex >= screens.Count) activeScreenIndex = -1;
            ApplyActiveScreen();
            onActiveScreenChanged?.Invoke(activeScreenIndex);
        }

        [Button("Toggle Current Screen")]
        public void ToggleCurrentScreen()
        {
            if (activeScreenIndex < 0 || activeScreenIndex >= screens.Count) return;
            var s = screens[activeScreenIndex];
            s.enabled = !s.enabled;
            screens[activeScreenIndex] = s;
            if (activeScreenIndex < screenQuads.Count && screenQuads[activeScreenIndex] != null)
            {
                screenQuads[activeScreenIndex].SetActive(s.enabled);
                if (activeScreenIndex < screenCameras.Count && screenCameras[activeScreenIndex] != null)
                    screenCameras[activeScreenIndex].enabled = s.enabled;
            }
        }

        private void ApplyActiveScreen()
        {
            for (int i = 0; i < screenQuads.Count; i++)
            {
                if (screenQuads[i] == null) continue;
                bool show = activeScreenIndex < 0 ? screens[i].enabled : (i == activeScreenIndex);
                screenQuads[i].SetActive(show);
                if (i < screenCameras.Count && screenCameras[i] != null)
                    screenCameras[i].enabled = show;
            }
            if (logEvents)
            {
                string name = activeScreenIndex < 0 ? "ALL" : screens[activeScreenIndex].name;
                Debug.Log($"[ScreenLayout] Active: {name}");
            }
        }
