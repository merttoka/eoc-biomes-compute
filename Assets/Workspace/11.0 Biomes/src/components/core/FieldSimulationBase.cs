...
namespace Biomes
{
    ...
    public abstract class FieldSimulationBase : SimulationBase
    {
        ...
        [Header("Centre keep-out (screen 1.2 cutout)")]
        [Tooltip("Normalized width of a centre band where this layer's contribution to the " +
                 ...
                 "never cross. 0 = off. Driven by ShowArc.centreKeepOut.")]
        [Range(0f, 0.6f)] public float centreKeepOut = 0f;
        [Tooltip("How much is removed at dead centre. Not 1 by default: the same master plays " +
                 "on a screen with no cutout, where a hard-edged empty band would read as a defect.")]
