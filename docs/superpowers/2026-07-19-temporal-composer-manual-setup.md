...
3. **Clear the main `SimulationManager`'s `compositeOutMat` now** (moved up from the old end-of-section step, right next to the wiring above, so the two-writer window never exists): if the main `SimulationManager` still has its own `compositeOutMat` pointed at `m_composite.mat`, clear it — only `CompositeSequencer` should write that material going forward (two writers racing on the same material per frame is a bug). *(Task 3 design intent)*
4. **`CellRig_A`.** Duplicate the existing `SimulationManager` GameObject hierarchy (manager + `Biome` + one or two sims — keep it light, e.g. just Physarum). Rename root `CellRig_A`.
   ...
   - On the duplicated rig hierarchy: clear `externalInput`, `neuronFiring`, and `injector` references, and **delete** any network/MIDI/OSC components (`ExternalTextureReceiver`/`ExternalTextureSender`, `MidiPianoMixer`, `OSCMapping`, `MidiFighterTwister`) found anywhere in the duplicate — duplicated ones would double-handle the same streams/MIDI/OSC alongside the originals.
...
5. **`CellRig_B`.** Duplicate `CellRig_A` → `CellRig_B`; give it a different snapshot preset (e.g. a Termite or Boid variant). Same network/MIDI/OSC cleanup as `CellRig_A` above applies to this duplicate too. Save as its own prefab. Spec caps at 4 cells — C/D optional, same pattern. *(Task 10)*
6. **`PlayableDirector` + `ShowSequence`.** On `TemporalComposer`: Add Component → `PlayableDirector`. In the Project window, `Assets/Workspace/11.2 SIGGRAPH Scene/assets/` → right-click → Create → Timeline → name `ShowSequence`; assign to the `PlayableDirector`. *(Task 5)*
7. **`BiomeCellTrack`.** With `TemporalComposer` selected, Window → Sequencing → Timeline. Add track → `Biomes` → `Biome Cell Track`; bind to `CompositeSequencer`. Add two `BiomeCellClip`s — A: `source = Rig`, `rig = CellRig_A`, `dstRect` = left band, `mode = Overlay`; B: `rig = CellRig_B`, `dstRect` = right band, `mode = Overlay`. ~10s each with 2–3s ease-in/out (drag clip edges). *(Task 5, sized per Task 10)*
8. **Reset signals.** Create `Assets/Workspace/11.2 SIGGRAPH Scene/assets/signals/`. Create → Signal ×4: `Sig_ResetSims`, `Sig_ResetPhysarum`, `Sig_ResetBoids`, `Sig_ResetTermites`.
...
9. **`PatchScatterTrack`.** Add track → `Patch Scatter Track` (orange), bind to `CompositeSequencer`. Add a clip: `sourceA = MainComposite`, `sourceB = DiffusionReturn`, `crossfadeCenter = 0.5` (defaults otherwise). *(Task 6, sourced per Task 10)*
10. **`ParamSnapshotTrack`.** Add track → `Param Snapshot Track`, bind to the **main** `SimulationManager`. Add a clip: `snapshot = assets/Snapshots/Physarum_20260711_195316`, `simIndex` = index of `PhysarumSim` in the manager's `simulations` list (check inspector), 10s ease-in. *(Task 7, timing per Task 10)*
11. **`RoutingTrack`.** Add track → `Routing Track`, bind to `CompositeSequencer`. Add a clip: `influenceSource = DiffusionReturn`. *(Task 8)*
12. **Second receiver (`TD_Diffusion`).** On the `TextureIO` GameObject (the one holding the existing receiver/sender; `NetworkIO` holds only OSC): add a second `ExternalTextureReceiver` — `enableReceive = ON`, `protocol = Syphon` (mac dev) / `Spout` (show machine), `streamName = "TD_Diffusion"`, `selfDrive = ON`. Assign it to `CompositeSequencer.diffusionReturn`. Assign the existing first receiver to `CompositeSequencer.inputReceiver`. *(Task 8)*
13. **Sender (`EoC/Composer`).** On `ExternalTextureSender`: add a stream with `source = ComposerOutput`, protocol Syphon (mac) / Spout (show), name left blank (defaults `EoC/Composer`); assign the `sequencer` field to the scene's `CompositeSequencer`. *(Task 8)*
14. **`ScreenLayout` re-point.** Confirm `ScreenLayout.screenMaterial` (and any other display/recorder path) is the same `m_composite.mat` that `CompositeSequencer.composerOutMat` now writes into every `LateUpdate` — i.e. displays and the outbound sender read `composerOutTex`, not the raw sim `compositeOutTex`, going forward (the main manager's `compositeOutMat` was already cleared in step 3). *(Task 3 design intent — spec: "Displays: ScreenLayout + senders sample composerOutTex, not the sim composite")*
15. **Full-frame replace clip.** Add a third `BiomeCellClip` on `BiomeCellTrack`: `rig = CellRig_A`, `mode = Replace`, `duckBase = ON`, `dstRect = (0, 0, 1, 1)` — full-frame takeover, fades back out per the 90s pass below. *(Task 10)*
