...
## Execution notes (2026-08-04, steps 2–7)

Deviations from the manifest above, and why. The manifest was a plan; where it conflicted
with the Verification commands, the commands won — as this document says they should.

1. **Archive head is `909a842`, not `92ff156`** (34 commits ahead, not 33). `92ff156` is the
   WIP commit; `909a842` is this spec correcting itself afterwards. Snapshots therefore take
   the corrected text. Local and `origin/` heads were SHA-verified equal before step 2.

2. **Step 5 was run against the archive, not the working tree.** As written it greps the
   scenes as they exist on `ca-dev` after C1 — but C3 *overwrites* those files with the
   archive versions, so the check as specified inspects the wrong blob. Both were checked;
   both are clean of `ShowArc` / `CueExporter` / `ShanghaiTransect` GUIDs, so C3 was ungated
   and took all four files. Also verified that the archive `Scene_SIGGRAPH.unity` holds zero
   references to the excluded `ShowSequence.playable`, `signals/` and `m_temporal_out.mat`,
   so importing it strands nothing.

3. **The session log was renamed and pruned.** C4's manifest lists
   `docs/sessions/2026-08-02-cellular-automata-dac-machinery.md`, whose *filename* matches
   the Shanghai-detection grep on `dac` — importing it verbatim fails this document's own
   verification gate. It landed as `docs/sessions/2026-08-02-cellular-automata-sims.md`,
   rewritten to the engine half, carrying a provenance note pointing at
   `archive/dac-shanghai` for the show half and the original combined log.

4. **Tooltip rewording went past the two named lines.** The same comment blocks named the
   venues — "Jingyao Hongqiao", "Xinda Plaza", "screen 1.2" — and `cellular_common.hlsl`
   carried three more of the same. The `centreKeepOut` mechanism travels; the venue prose
   does not. All are now phrased generically ("a central cutout in the physical display").

5. **Four docs came along that C4 did not list**, all engine or engine-adjacent and none
   Shanghai: `ROADMAP.md` (a Todoist scope note from `e1f69b1`), the MFT extraction spec and
   the neuron-layout spec (the design doc for C1's own change), and `ARCHITECTURE.md`
   wholesale — verified to contain no show references, so the hand-port C4 anticipated was
   unnecessary. `README.md` *was* hand-ported: the 15-channel line and the CA pillar came
   across, the `Scene_DAC — Shanghai (11.3)` bullet did not.

6. **Two wikilinks broke and were repaired**: ADR-0011 and the composer-bioform plan pointed
   at the renamed session log and at the archived Shanghai spec respectively. A sweep
   confirms every remaining `[[wikilink]]` under `docs/` resolves, except three that were
   already broken on `origin/main` and are out of scope here.
